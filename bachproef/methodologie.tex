...
Omdat er weinig publieke testdata beschikbaar is, wordt een eigen testomgeving ontwikkeld: de \emph{Fintech MCP Goat}, een opzettelijk kwetsbare MCP-server die financiële scenario's nabootst (saldo opvragen, overschrijving starten). Deze server dient als gecontroleerde \emph{ground truth} met vijf bekende, opzettelijk ingebedde kwetsbaarheden. De evaluatie steunt op deze ene gecontroleerde case; bredere uitspraken over andere MCP-servers vallen buiten de scope van dit onderzoek. Parallel wordt een gestructureerde handmatige beoordelingsprocedure vastgelegd als checklist, zodat de vergelijking reproduceerbaar is. Per testscenario worden vier grootheden gemeten:
...
\begin{itemize}
    ...
    \item \textbf{Precisie:} Aandeel terechte bevindingen op het totaal aantal gerapporteerde bevindingen
...
De testomgeving draait via Docker~Compose. Het resultaat is een validatierapport met kwantitatieve meetresultaten voor de Goat-server. Geschatte doorlooptijd: drie weken.
