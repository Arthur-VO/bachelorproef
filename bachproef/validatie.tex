...
\section{Experimentele opstelling}
\label{sec:opstelling}

Alle experimenten zijn uitgevoerd op een HP laptop met een Intel Core i7-1165G7 (4~cores, 2{,}80~GHz), 16~GB RAM en Windows~11~Home (build 26200). Docker~Desktop~28.4.0 met WSL2-backend (WSL-kernel 6.6.87.2) verzorgde de containeromgeving; de Goat-server en de scanner draaiden als twee aparte containers op een geïsoleerd intern netwerk (\texttt{mcp-lab-net}). Python~3.13 en de volgende bibliotheken werden gebruikt: \texttt{httpx~0.28.1}, \texttt{pydantic~2.11.7} en \texttt{PyYAML~6.0.2}. De volledige \texttt{docker-compose.yml} is opgenomen in bijlage~\ref{app:compose}. De evaluatie is uitgevoerd op commit \texttt{1247c41} van de scannerrepository (mei 2026).

De geautomatiseerde scans werden drie keer opeenvolgend uitgevoerd na een koude opstart van de containers (\texttt{docker compose up -{}-build}). De gerapporteerde uitvoeringstijd van minder dan 30~seconden is het gemiddelde van deze drie runs, gemeten als wallclock-tijd van \texttt{docker compose run}. De handmatige procedure werd op dezelfde machine uitgevoerd door één beoordelaar en bestond uit een code review van \texttt{server.py} gevolgd door actieve HTTP-tests via \texttt{curl}; de tijd werd bijgehouden met een stopwatch en bedroeg 47~minuten. Beide procedures zijn uitgevoerd in mei 2026.
...
\subsection*{Precisie en false positives}

Van de 13 bevindingen is er één debateerbaar: FUZZ-002 op \texttt{list\-\_re\-cent\-\_trans\-ac\-tions}. De fuzzer detecteerde dat een injection-payload ongesaniteerd terugkomt in de response van deze tool. Dit is geen opzettelijk ingebedde kwetsbaarheid, maar de scanner signaleert terecht dat de server elke willekeurige string als \texttt{account\_id} terugkaatst. Afhankelijk van de definitie telt dit als een echte vinding of als een false positive. In het meest conservatieve geval bedraagt de \textbf{precisie} $\frac{12}{13} \approx 92{,}3\%$. Een klassieke false positive rate (FP / (FP + TN)) is niet van toepassing in een vulnerability scan, omdat true negatives (tools die geen kwetsbaarheid bevatten en ook niet gemarkeerd worden) niet zinvol te tellen zijn.
...
\begin{table}[htbp]
    \centering
    \caption{Resultaten handmatige beoordeling.}
    \label{tab:manueel}
    \begin{tabular}{@{}lll@{}}
        \toprule
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        Precisie                & $\geq$92,3\% (12/13) & 100\% (4/4) \\
